...
def Makexyzdistance(t):
    ...
    z = np.loadtxt(t, dtype=float, usecols=(3), skiprows=2, max_rows=atoms_num)
...
def VariancePersistv1(
    ...
    for index in points:
        ...
        #  print (c)
        eleneg.append(
            np.abs(ELEMENTS[elements[c[0]]].eleneg - ELEMENTS[elements[c[1]]].eleneg)
        )
    ...
    h0matrix = np.hstack(
        (
            (
                diagrams[0][0:-1, :],
                np.reshape(((np.array(eleneg) + 0.4) / 10), (np.size(eleneg), 1)),
            )
        )
    )
    ...
    pim = PersImage(
        pixels=[pixelx, pixely], spread=myspread, specs=myspecs, verbose=False
    )
    ...
    h0matrix = np.hstack(
        (
            (
                diagrams[0][0:-1, :],
                np.reshape(
                    (((np.array(eleneg) * 1.05) + 0.01) / 10), (np.size(eleneg), 1)
                ),
            )
        )
    )
